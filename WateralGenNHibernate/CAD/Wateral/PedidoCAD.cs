                                ...
                                )
{
        ...
        try
        {
                ...
                if (ex is WateralGenNHibernate.Exceptions.ModelException)
                        throw ex;
                throw new WateralGenNHibernate.Exceptions.DataLayerException ("Error in PedidoCAD.", ex);
        }
...
public void ModifyDefault (PedidoEN pedido)
{
        try
        {
                ...
                pedidoEN.Precio_total = pedido.Precio_total;
...
public int New_ (PedidoEN pedido)
{
        try
        {
                ...
                if (pedido.LineasPedido != null) {
                        foreach (WateralGenNHibernate.EN.Wateral.LineaPedidoEN item in pedido.LineasPedido) {
                                item.Pedido = pedido;
                                session.Save (item);
                        }
                }
                if (pedido.UserRegistrado != null) {
                        // Argumento OID y no colección.
                        pedido.UserRegistrado = (WateralGenNHibernate.EN.Wateral.UserRegistradoEN)session.Load (typeof(WateralGenNHibernate.EN.Wateral.UserRegistradoEN), pedido.UserRegistrado.Email);

                        pedido.UserRegistrado.Pedidos
                        .Add (pedido);
                }
